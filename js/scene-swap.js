...
class SwapScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(cx, L.headTop, '무기를 찾았습니다', font(21, '#8794b5')).setOrigin(0.5);
    ...
    this.card(cx + half, cardTop, '새로 찾은 것', this.entry, false, 0x7e6bc4);
    ...
    this.add.text(cx, lostY, lost.length
      ? '갈아타면 잃습니다 —  ' + lost.join('   ')
      : '아직 잃을 강화가 없습니다', font(18, lost.length ? '#ff8a80' : '#5c6890')).setOrigin(0.5);
