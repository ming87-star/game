...
// 「~로도」인지 「~으로도」인지. 받침이 있으면 '으로', 없으면 '로'입니다.
// (ㄹ 받침만은 예외로 '로'를 씁니다 — "서울로"이지 "서울으로"가 아닙니다.)
//
// 직업 이름을 글에 끼워 넣는 자리가 여기뿐이라 작게 둡니다. 그냥 '으로도'로
// 못박으면 "궁수으로도"가 됩니다 — 화면에 그대로 나오는 글이라 티가 납니다.
function withRo(word) {
  const code = word.charCodeAt(word.length - 1);
  const isHangul = code >= 0xac00 && code <= 0xd7a3;
  const jong = isHangul ? (code - 0xac00) % 28 : 0;
  return word + (jong === 0 || jong === 8 ? '로도' : '으로도');
}
