...
const RELIC_ART = {
  ...
  // 병은 아래에 내려놓습니다.
  // 관통하는 기름 — 예전에는 **깃까지 달린 화살**이었습니다. 공용 유물인데
  // 그림만 보면 활잡이의 것이라, 근접 넷은 카드가 뜨는 순간 넘겼습니다.
  // 지금은 갈래를 안 가리는 표로 그립니다 — **두 겹을 뚫고 나온 뾰족한 것**.
  // 깃이 없어야 화살로 안 읽히고, 판이 둘이라야 「하나 더」가 읽힙니다.
  piercingoil: (g) => {
    // 뚫린 판 둘. 앞의 것은 밝고 뒤의 것은 어둡습니다 — 뒤가 뒤로 보이게.
    // 자리는 뜨거운·차가운 기름과 같게 잡습니다 — 표는 위, 병은 아래.
    // 셋이 한 줄에 서면 「같은 기름 세 자매」로 읽혀야 합니다.
    // 뚫린 널 둘. **쇠와 다른 색이라야** 48px 에서 「뚫렸다」가 보입니다 —
    // 같은 쇠빛으로 그렸더니 가로줄 하나로 뭉쳐 보였습니다.
    // 병이 먼저, 표가 그 위 — 뜨거운 기름과 같은 차례입니다.
    rFlask(g, 0x8bc34a, 24, 34);
    // 뚫린 널 둘을 **가로로** 눕힙니다. 세로로 세우면 꿰뚫은 것에 가려
    // 48px 에서 가로줄 하나로 뭉쳐 보였습니다(처음 그린 것이 그랬습니다).
    rFill(g, 0x795548);
    g.fillRect(9, 8, 24, 6);
    g.strokeRect(9, 8, 24, 6);
    rFill(g, 0x5d4037);
    g.fillRect(13, 16, 17, 5);
    g.strokeRect(13, 16, 17, 5);
    // 둘을 내리꽂고 나온 못. 머리는 **둥글게** 그립니다 — 네모난 머리에
    // 곧은 날을 붙였더니 단검으로 읽혔습니다(그러면 활 쪽이 또 남습니다).
    // 못은 어느 자루에도 매이지 않은 물건입니다.
    rFill(g, 0xc3d4e4);
    iconPoly(g, [[21, 3], [27, 3], [25, 19], [24, 25], [23, 19]]);
    rBlob(g, 24, 3, 5, 0xb0bec5);
  },
