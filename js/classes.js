...
const CLASSES = [
  ...
  },
  {
    ...
    // **셋이 다 붙으면 주인공 초당 피해의 73%** 입니다 (node thrall-check.js).
    // 예전에는 47% 였고 그때 1.15 로 잡았습니다.
    //
    // 그렇다고 1.73 은 아닙니다 — 늘 셋인 것이 아니라 잡아서 세우는 것이고,
    // 맞으면 깎이고 다 깎이면 스러집니다. 셋이 서 있는 시간을 6할쯤으로
    // 보고 1 + 0.73 × 0.6 ≈ 1.44 로 잡았습니다.
    표에안잡힘: '셋을 채워 두면 주인공 초당 피해의 73%가 더 붙습니다',
    그럴듯: 1.45,
    name: '사령술사',
  ...
  },
  {
    ...
    weapons: [
      ...
      // 공격력이 51 이었습니다. 마법사 자루 열셋 중 **둘째로 높은** 값인데
      // 거기에 꿰뚫기 3 과 번개 2 를 얹어 놨습니다. 재 보니 세 마리에게
      // 206 이 들어가서, 깊이 300 짜리가 깊이 450(별의 218)급 출력을
      // 냈습니다 — 화염폭풍(350, 152)도 서리(400, 61)도 넘습니다.
      // **배수를 둘 지닌 자루에 높은 기본값까지 주면 안 됩니다.** 절반으로
      // 내립니다 (node spell-check.js 로 다시 잽니다).
      { key: 's7', name: '사슬 지팡이', dmg: 26, rate: 283, range: 395, depth: 300,
  ...
  },
  {
    ...
    relicMax: 5,
    // ── 둘을 골라 들고 시작합니다 ────────────────────────
    // 유물은 200층부터 100층마다 하나뿐이라, 다섯 칸은 **600층에 가야**
    // 다 찹니다. 그때까지 도굴꾼은 남들과 똑같이 둘을 든 채 남들보다 낮은
    // 수치로 오릅니다 — `node relic-pace.js` 로 재면 0층에서 가장 센
    // 곰사냥꾼의 39%, 400층까지도 47% 입니다. 「다섯을 드는 사람」이
    // 다섯을 들기 전에 죽습니다.
    //
    // 오르기 전에 둘을 고릅니다(scene-game.js 의 openStartRelic). 그러면
    // 0층 39% → 55%, 다 채우는 층 600 → 400 입니다. 끝값은 그대로라
    // job-scale.js 의 94 는 움직이지 않습니다 — 고쳐진 것은 **앞부분**입니다.
    startRelics: 2,
