...
const CLASSES = [
  ...
  //
  // 해금 조건은 **그 자리 순서 그대로**입니다. 위에 앉을수록 어렵게 열립니다
  // (도굴꾼 300층 → 곰사냥꾼 1000층). 전사가 공짜라 바닥인 것과 같은 결입니다.
  //
  // ── 아직 안 붙은 것 셋 ──────────────────────────────────
  ...
  // 능력이 없으면 표에서 잰 자리보다 약합니다. 붙이기 전까지는 그렇습니다.
  {
    ...
    // 짐작한 배수를 곱해서 점수를 맞추면 어떤 답이든 나오므로, 배수를
    // 여기 적어 두고 **셈이 그 값으로 앉는 자리를 내게** 합니다.
    // 가만히 서서 때려야 쌓이는데 이 게임에서 할 일은 올라가는 것이라,
    // 층을 옮길 때마다 풀립니다. 한 발판에서 서너 대가 보통입니다.
    표에안잡힘: '연타가 쌓이면 세집니다',
    그럴듯: 1.3,
    name: '권법사',
    unlockFloor: 400, unlockCoins: 1200,
    rumor: '무기 없이 오르는 사람이 있다고 합니다.',
    blurb: '맨손으로 이어 친다',
    detail: '무기를 안 씁니다.\n이어 칠수록 세지고, 자리를 옮기면 처음으로 돌아갑니다.',
    color: 0xffd54f,
    ...
    weapons: [
      ...
      { key: 'stone', name: '돌주먹', dmg: 36, rate: 168, reach: 72, depth: 200,
      ...
      { key: 'sky', name: '천공권', dmg: 44, rate: 130, reach: 88, depth: 500,
  ...
  },
  {
    ...
    // 짐작한 배수를 곱해서 점수를 맞추면 어떤 답이든 나오므로, 배수를
    // 여기 적어 두고 **셈이 그 값으로 앉는 자리를 내게** 합니다.
    // 내 화력에 더해지는 것이 아니라 **덜 마주치게** 하는 것이라 두 배까지는
    // 아닙니다.
    ...
    blurb: '곰을 앞서 보낸다',
    detail: '곰이 먼저 올라가 싸웁니다. 나는 뒤에서 활을 씁니다.\n한 발이 무겁고 느립니다.',
    color: 0xbcaaa4,
  ...
  },
  {
    ...
    // 짐작한 배수를 곱해서 점수를 맞추면 어떤 답이든 나오므로, 배수를
    // 여기 적어 두고 **셈이 그 값으로 앉는 자리를 내게** 합니다.
    // 셋이 각자 내 화력의 3할씩만 해도 +0.9배입니다. 다만 셋을 늘 채워 두려면
    // 계속 잡아야 하고, 층을 옮기면 따라오는 데 시간이 걸립니다.
  ...
  },
  {
    ...
    그럴듯: 1.0,
    name: '도굴꾼',
    unlockFloor: 300, unlockCoins: 800,
