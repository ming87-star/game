...
function buildTextures(scene) {
  ...
  // ── 붉은 겉옷 (33층 시퀀스) ────────────────────────────
  //
  // 이 게임에서 **붉은 것은 이것 하나뿐입니다** (STORY.md 3절). 전사의
  // 붉은 망토도 군청으로 옮겼고, 이야기 그림 여섯 장도 다시 그렸습니다.
  // 상징이 둘이면 상징이 아닙니다.
  //
  // ── 얼굴을 어디까지 보일까 ─────────────────────────────
  //
  // 한동안 후드 안이 **검은 타원 하나**였습니다. 「누구인지 특정하는 순간
  // 어떤 사람의 이야기가 된다」는 까닭이었는데, 그리고 나서 보니 값을
  // 잘못 치렀습니다 — 얼굴이 아예 없으니 **나쁜 것**으로 읽힙니다.
  //
  // 하필 이 시퀀스에서 그를 죽이는 「내려온 것」도 얼굴 없는 두건입니다
  // (art/ending-foe.svg). 둘이 같은 문법을 쓰고 있었던 것입니다.
  //
  // ── 얼굴을 그리지 않고 빛을 하나 놓습니다 ──────────────
  // 후드 안쪽은 10×9 픽셀입니다. 눈코입은 안 들어갑니다. 대신 **턱에 닿는
  // 빛 한 조각**만 놓습니다. 눈은 그늘에 남습니다 — 사람인 것은 알겠고
  // 누구인지는 모르는 자리가 거기입니다.
  //
  // 그리고 이 하나로 놈과 갈립니다. 빛이 어디에 있느냐가 전부입니다.
  //
  //   내려온 것   눈만 밝고 턱이 없습니다   (빛이 위에)
  //   붉은 옷     턱만 밝고 눈이 없습니다   (빛이 아래)
  //
  // ── 후드 안쪽 값은 옷마다 다릅니다 ─────────────────────
  // 붉은 옷은 횃불뿐인 탑 안에 있으니 굴이 거의 검습니다. 흰옷은 탑 밖
  // 대낮입니다 — 여기서까지 굴을 검게 두면, 밝은 하늘 앞에 **검은 구멍**이
  // 하나 뚫린 것으로 보입니다. 실제로 그랬습니다. 대낮의 후드 그늘은
  // 검정이 아니라 회색입니다.
  //
  // 손그림이 오면 이 셋만 load.image() 로 바꾸면 됩니다.
  const 겉옷 = (key, 천, 안감, 살, 살그늘, 굴, 이마) => {
    g.clear();
    g.fillStyle(0x1a1c26, 1);
    g.fillRoundedRect(11, 16, 14, 26, 4);        // 몸 — 어둡습니다
    g.fillStyle(안감, 1);
    iconPoly(g, [[7, 40], [29, 40], [26, 18], [10, 18]]);   // 겉옷 안쪽
    g.fillStyle(천, 1);
    iconPoly(g, [[5, 44], [31, 44], [27, 14], [9, 14]]);    // 겉옷
    g.fillStyle(천, 1);
    g.fillRoundedRect(10, 4, 16, 14, 7);         // 후드
    g.fillStyle(굴, 1);
    g.fillEllipse(18, 12, 10, 9);                // 후드 안 — 그늘

    // 얼굴을 **둥근 덩어리**로 놓고 위쪽만 다시 덮습니다.
    //
    // 처음에는 턱을 사다리꼴로 그렸습니다. 후드 폭을 거의 다 채우는 가로
    // 조각이라 얼굴이 아니라 **우편함 투입구**로 보였습니다. 사람 얼굴은
    // 아래가 좁고 둥급니다 — 그 하나만 지키면 10픽셀에서도 얼굴이 됩니다.
    g.fillStyle(살그늘, 1);
    g.fillEllipse(18, 13.6, 7.6, 8);
    g.fillStyle(살, 1);
    g.fillEllipse(18, 14.4, 6, 6.2);
    // 후드가 이마에 드리우는 그늘. **눈이 있어야 할 자리를 덮습니다** —
    // 남는 것은 코끝 아래와 턱뿐입니다. 사람인 건 알겠고 누구인지는 모릅니다.
    g.fillStyle(이마, 0.92);
    g.fillEllipse(18, 11.4, 11, 7.4);
    bake(key, 36, 46);
  };
  겉옷('cloak-red', 0xc62828, 0x8e1f1f, 0xd9a284, 0x9c6a50, 0x0d0f16, 0x0d0f16);
  겉옷('cloak-white', 0xf5f5f5, 0xcfd8dc, 0xf7d9bd, 0xc9a084, 0x6f7480, 0x4b505c);

  // 벗겨진 겉옷 — 사람 없이 천만. 떨어지고, 바닥에 놓입니다.
  g.clear();
  g.fillStyle(0x8e1f1f, 1);
  iconPoly(g, [[4, 20], [30, 20], [26, 9], [8, 9]]);
  g.fillStyle(0xc62828, 1);
  iconPoly(g, [[2, 22], [32, 22], [27, 5], [7, 5]]);
  g.fillStyle(0x8e1f1f, 1);
  g.fillRoundedRect(9, 0, 16, 9, 4);             // 접힌 후드
  bake('cloak-fallen', 34, 24);
