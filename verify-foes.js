...
(async () => {
  ...
  const fierce = await page.evaluate(() => {
    ...
    const make = (floor) => {
      ...
      const out = { hp: e.maxHp, dmg: e.contactDamage, fierce: !!e.fierce };
      e.destroy();
      return out;
    };
    ...
    const low = make(300);
    return { plain, wild, low, want: CFG.foes.fierce };
  });
  ...
  check(Math.abs(fierce.wild.hp / fierce.plain.hp - fierce.want.hp) < 0.06,
    '두 바퀴째 체력이 정해진 배수',
    (fierce.wild.hp / fierce.plain.hp).toFixed(2) + '배 / ' + fierce.want.hp + '배');
