...
function measure(job, f) {
  ...
  // field  닿은 자리에 깔리는 장판 한 판의 몫. 맞은 놈은 대개 그 자리에
  //        서 있으므로 화상과 같은 자리로 봅니다 — 다만 걸어 나가면
  //        끊기고, 남이 걸어 들어오면 그쪽도 맞습니다. 후자는 「여럿에게
  //        닿는 것」이라 여기 안 잡히고 「그럴듯」이 답니다
  ...
  const each = pool
    .map((x) => (x.dmgMin + x.dmgMax) / 2 * boost * 발사체(x.shots) * x.acc
      * (1 + (x.burn || 0) + (x.field || 0)) * 연타 / (x.rate / speed) * 1000)
    .sort((a, b) => a - b);
