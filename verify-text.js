...
(async () => {
  ...
  const out = await page.evaluate(() => {
    ...
    // 만남 컷 (js/scene-meet.js) — 그림 틀이 452 이고 글은 그 아래
    // 가운데 맞춤입니다. 틀보다 넓으면 그림과 글의 폭이 어긋나 보입니다.
    //
    // **여기가 오래 비어 있었습니다.** 직업을 여덟으로 늘리면서 다섯의 글을
    // 새로 썼는데, 이 화면은 재는 자리가 아예 없어서 첫 줄이 틀 밖으로
    // 삐져나온 채로 지나갈 뻔했습니다.
    Object.entries((CFG.story && CFG.story.meetings) || {}).forEach(([key, t]) => {
      put('만남 제목', key, t.title, 30, 452);
      (t.lines || []).forEach((줄, i) => put('만남 ' + (i + 1) + '줄', key, 줄, 20, 452));
    });
