...
(async () => {
  ...
  await page.waitForTimeout(300);
  t = await texts();
  const 도적줄 = t.find((x) => x.indexOf('700층') >= 0) || '없음';
  check(t.some((x) => x.indexOf('700층') >= 0 && x.indexOf('2000') >= 0),
    '잠긴 직업도 해금 조건은 그대로 보여 줌', 도적줄);

  // ── 「누구로 올라야 하는지」가 빠지면 안 됩니다 ─────────
  //
  // 해금은 사슬입니다 — 아무 직업으로나 채우는 것이 아니라 **정해진 한
  // 사람**으로 올라야 열립니다. 한동안 앞 사람까지 잠긴 칸에는 「한 판에서
  // 700층 · 코인 2000」만 적혀 있었고, 그러면 **아무 직업으로나 채우면
  // 되는 것으로 읽힙니다.** 새 저장에서 잠긴 일곱 중 여섯이 그 줄을 봤습니다.
  //
  // 이름은 여전히 감춥니다(잠긴 칸이 「???」인데 옆에서 흘리면 뜻이 없습니다).
  // 감추는 것은 **누구인가**이지 **누군가로 올라야 한다**가 아닙니다.
  check(/(으?로)\s/.test(도적줄),
    '조건 줄에 「누구로 올라야 하는지」가 들어 있음', 도적줄);
