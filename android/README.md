...
### ②-2 두 번째부터는 **버전 코드를 올려야 합니다**

`android/app/build.gradle` 의 두 줄입니다.

```gradle
versionCode 1        // 올릴 때마다 +1. **한 번 쓴 숫자는 다시 못 씁니다**
versionName "1.0"    // 사람이 읽는 이름
```

안 올리고 올리면 콘솔이 「이 버전 코드는 이미 사용되었습니다」로 막습니다.

**같은 빌드를 다른 트랙에 얹을 때는 다시 굽지 마세요.** 내부 테스트에
올린 것을 비공개 테스트로 보내려면 버전을 만들 때 **「라이브러리에서
추가」** 로 이미 올라간 번들을 고르면 됩니다. 다시 구우면 다른 파일이
되어서, 테스터가 받는 것이 내가 확인한 것과 달라집니다.
...
- [ ] **스토어 설명** → 「없는 것」 대목에 「광고가 없습니다」가 있습니다
      (store/listing-ko.md · 간단한 설명에도 들어 있습니다)
