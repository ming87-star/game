...
// 겉옷 둘은 **2로만 나눕니다.** 4로 나누면 34×24 · 34×36 이 되는데,
...
// ── ⚠ 이 PNG 셋을 덮어쓰지 마세요 ─────────────────────────
//
// assets/player-warrior|archer|rogue.png 를 c7809f3 에서 **실수로 옛
// 그림으로 덮어썼습니다.** 그 커밋은 0층 바닥을 놓는 일이었고 초상화는
// 건드릴 일이 없었습니다. 54519 → 17646 바이트로 줄었는데 아무도 안 봤고,
// 배포된 판에서 고르는 화면의 전사와 들어가서 만나는 전사가 **딴 사람**이
// 되어서야 드러났습니다. 오류는 안 났습니다. 파일이 작아졌을 뿐입니다.
//
// 지금 있는 것이 원래 그림입니다 (시트와 같은 사람). 되돌린 자리:
//   git checkout c7809f3^ -- assets/player-warrior.png ...
const PORTRAITS = [
